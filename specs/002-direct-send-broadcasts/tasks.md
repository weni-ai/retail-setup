...
> ## ⚠️ OUTSTANDING WORK BEFORE MERGE (pinned 2026-05-22 by `/speckit-analyze`)
>
> Phase 7 (T036–T039) is `[X]` (initial PR opened, coverage parity verified) — but FR-003g, FR-014a, and FR-014b were appended to `spec.md` AFTER Phase 7 closed and were folded in by the Phase 8 extension. **Three Phase-8-extension tasks are still `[ ]`**:
>
> - **T112** (FR-003g override map) — `DIRECT_SEND_BUTTON_LABEL_OVERRIDES` does not yet exist in `retail/`; `_validate_and_normalize_buttons` doesn't accept the `language` argument required for the lookup. **Real-world `order_canceled_3` × `pt_BR` / `es` assignments will atomic-rollback at FR-003d** until this lands — exactly the failure mode FR-003g was added to fix.
> - **T113** (FR-014a CTA URL wire shape) — `Broadcast.build_direct_send_message` still writes `msg["buttons"] = buttons` and `_exceeds_direct_send_length_limits` still iterates `msg.buttons[*]`. **Every Direct Send dispatch with a CTA URL today violates FR-014a** on the wire.
> - **T114** (FR-014b QUICK_REPLY wire shape) — same code path. Combined with FR-014a(b) + FR-014b(b), the Direct Send path MUST NOT emit `msg.buttons` at all; today it does.
>
> **Hard pre-merge gate** (Constitution Principle III — Test Coverage Parity & Isolated Tests): T112 + T113 + T114 MUST land in the same merge window as the rest of this feature. After they land, **T037 (`compare_coverage.py`) and T039 (PR body) MUST be re-run / re-verified** — see the inline notes on those tasks. T115 is `[X]` because the contract artifact is already canonical; the implementation is what trails.
>
> **Coverage state pre-T112/T113/T114** (today): `[X]`-marked tasks T011, T011c, T013 cover the pre-FR-014a/FR-014b `msg.buttons[*].sub_type` shape only; the canonical `msg.interaction_type` / `msg.cta_message` / `msg.quick_replies` shape currently has zero test coverage in the repository. T011a is `[~] SUPERSEDED by T114` but T114 is pending, so QUICK_REPLY canonical-shape coverage is also currently zero. This is the regression surface the merge gate is protecting against.
...
- [ ] T113 [US1] **FR-014a — CTA URL wire shape relocation (Session 2026-05-22 Q4)** — Tests in `retail/agents/tests/services/test_broadcast_direct_send.py` AND `retail/agents/tests/services/test_direct_send_payload_builder.py` AND dispatch implementation in `retail/agents/domains/agent_webhook/services/broadcast.py` + `retail/agents/domains/agent_webhook/services/direct_send_payload_builder.py`. **Tests** (one method per branch):
...
- [ ] T114 [US1] **FR-014b — QUICK_REPLY wire shape relocation (Session 2026-05-22 Q10)** — **SUPERSEDES T011a**. Tests in `retail/agents/tests/services/test_broadcast_direct_send.py` AND `retail/agents/tests/services/test_direct_send_payload_builder.py` AND dispatch implementation in `retail/agents/domains/agent_webhook/services/broadcast.py` + `retail/agents/domains/agent_webhook/services/direct_send_payload_builder.py`. **Tests** (one method per branch):
