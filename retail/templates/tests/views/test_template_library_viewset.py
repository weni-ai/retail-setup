...
class TestTemplateLibraryViewSet(APITestCase):
    ...
    def test_partial_update_success(self):
        with patch(
            ...
            self.update_library_usecase.get_template.return_value = self.template
            ...
            self.update_library_usecase.get_template.assert_called_once_with(
                str(self.template.uuid)
            )
    ...
    def test_partial_update_no_project_header(self):
        with patch(
            "retail.templates.views.UpdateLibraryTemplateUseCase",
            return_value=self.update_library_usecase,
        ):
    ...
    def test_partial_update_wrong_project_header(self):
        with patch(
            "retail.templates.views.UpdateLibraryTemplateUseCase",
            return_value=self.update_library_usecase,
        ):
            ...
            response = self.client.patch(
                ...
                payload,
                format="json",
                headers={"Project-Uuid": str(other_project.uuid)},
            )

            self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
            self.update_library_usecase.execute.assert_not_called()

    def test_partial_update_template_without_integrated_agent(self):
        with patch(
            "retail.templates.views.UpdateLibraryTemplateUseCase",
            return_value=self.update_library_usecase,
        ):
            template_without_integrated_agent = Template.objects.create(
                uuid=uuid4(),
                name="test_template_no_integrated_agent",
                parent=self.parent,
            )

            self.update_library_usecase.get_template.return_value = (
                template_without_integrated_agent
            )
            ...
            url = reverse(
                ...
                args=[str(template_without_integrated_agent.uuid)],
            )
            url += f"?app_uuid={self.app_uuid}&project_uuid={self.project_uuid}"
