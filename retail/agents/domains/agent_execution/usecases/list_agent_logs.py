...
Distinct from ``ListExecutionsUseCase`` because the agent-logs API
surface differs: page/page_size + total instead of limit/offset,
ILIKE search across two fields, multi-status and multi-template OR
filters, and a date filter that is a single calendar day rather than
an arbitrary range. Keeping the existing internal use case unchanged
means admin tools and CLI scripts that depend on it stay untouched.
"""
